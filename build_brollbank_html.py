...
<body class="min-h-screen selection:bg-purple-500 selection:text-white flex flex-col bg-[#0b0f19]">
    ...
    <nav class="sticky top-0 z-40 glass border-b border-slate-800 px-4 lg:px-8 py-3.5 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            <a href="http://127.0.0.1:8899/brollbank.html" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600/30 text-emerald-300 border border-emerald-500/50 hover:bg-emerald-600 hover:text-white text-xs font-bold shadow-lg transition" title="Mở bản Quản trị trực tiếp trên máy Mac (Xóa 1-click tức thì)">
            ...
            <a href="{db.get('gdrive_folder_url', 'https://drive.google.com/open?id=1R4Wyl_c8MxLPqBJRR-5Dc5I3P3Hb7tSA')}" target="_blank" class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-xs font-bold shadow-lg shadow-blue-600/30 transition">
            ...
            <a href="https://www.youtube.com/playlist?list=PLPs82ezbs9Lo" target="_blank" class="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-600 text-white hover:bg-red-700 text-xs font-bold shadow-lg shadow-red-600/30 transition">
            ...
            <a href="https://fedu.vn/course/slide-8-loai-broll-video-course.html" target="_blank" class="hidden lg:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600/30 text-indigo-300 border border-indigo-500/40 hover:bg-indigo-600 hover:text-white text-xs font-semibold transition">
                8 Loại B-Roll Fedu
            </a>
        </div>
    </nav>
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 bg-black/85 backdrop-blur-md hidden flex items-center justify-center p-4 sm:p-6 opacity-0 transition-opacity duration-300">
        <div class="glass max-w-md w-full rounded-2xl border border-red-500/40 flex flex-col overflow-hidden shadow-2xl bg-slate-900">
            ...
            <div class="p-5 sm:p-6 space-y-4">
                ...
                <div id="del-status-box" class="hidden p-3.5 rounded-xl text-xs space-y-2.5">
                    <div id="del-status-text" class="font-medium leading-relaxed"></div>
                    <div id="del-cli-fallback" class="hidden space-y-2 pt-1 border-t border-red-500/20">
                        <a href="http://127.0.0.1:8899/brollbank.html" target="_blank" class="w-full py-2.5 px-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-900/30 transition">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14.5v-9l6 4.5-6 4.5z"/></svg>
                            Mở Bản Quản Trị Trực Tiếp (Xóa 1-Click Tức Thì)
                        </a>
                        <div class="text-[11px] text-slate-400 pt-1">Hoặc sao chép lệnh Terminal để chạy ngay:</div>
                        <div class="flex items-center gap-2">
                            ...
                            <button onclick="copyCliCmd()" class="px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold shrink-0 border border-slate-700">Copy</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let API_BASE = (window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost') ? '' : 'http://127.0.0.1:8899';
        const CANDIDATE_ENDPOINTS = [
            (window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost') ? '' : 'http://127.0.0.1:8899',
            'http://127.0.0.1:8899',
            'http://localhost:8899'
        ].filter(Boolean);
        ...
        async function checkApiStatus() {{
            ...
            if (window.location.hostname === '127.0.0.1' || window.location.hostname === 'localhost') {{
                badge.innerHTML = '<span class="text-emerald-400 font-semibold">🟢 Local App Sẵn Sàng (Port 8899)</span>';
                return;
            }}
            for (const ep of CANDIDATE_ENDPOINTS) {{
                try {{
                    const res = await fetch(`${{ep}}/api/status`, {{ method: 'GET', signal: AbortSignal.timeout(1200) }});
                    if (res.ok) {{
                        API_BASE = ep;
            ...
            if (window.location.protocol === 'https:') {{
                badge.innerHTML = '<a href="http://127.0.0.1:8899/brollbank.html" target="_blank" class="text-amber-300 hover:text-white font-semibold underline">⚠️ Mở Bản Local Để Xóa 1-Click</a>';
            }} else {{
        ...
        async function executeDeleteBroll() {{
            ...
            statusBox.className = 'p-3.5 rounded-xl text-xs bg-slate-950 border border-purple-500/40 text-purple-200 block space-y-2.5';
            statusText.innerText = '📡 Đang gọi YouTube API và đồng bộ cơ sở dữ liệu...';

            const endpointsToTry = API_BASE ? [API_BASE, 'http://127.0.0.1:8899', 'http://localhost:8899'] : ['', 'http://127.0.0.1:8899', 'http://localhost:8899'];
            let deletedOk = false;
            let lastErr = null;

            for (const ep of endpointsToTry) {{
                try {{
                    const url = ep ? `${{ep}}/api/delete-broll` : '/api/delete-broll';
                    ...
                    if (res.ok && data.success) {{
                        API_BASE = ep;
                        deletedOk = true;
                        statusBox.className = 'p-3.5 rounded-xl text-xs bg-emerald-950/40 border border-emerald-500/40 text-emerald-200 block';
                        ...
                        lastErr = data.error || 'Lỗi từ server';
                    }}
                }} catch (e) {{
                    lastErr = e.message;
                }}
            ...
            if (!deletedOk) {{
                statusBox.className = 'p-3.5 rounded-xl text-xs bg-red-950/40 border border-red-500/40 text-red-200 block space-y-2.5';
                if (window.location.protocol === 'https:') {{
                    statusText.innerHTML = `⚠️ <b>Trình duyệt chặn kết nối từ HTTPS sang Local HTTP (Mixed Content).</b><br>Để xóa 1-click tức thì, anh mở <b>Bản Quản Trị Trực Tiếp</b> trên máy nhé:`;
                }} else {{
                    statusText.innerHTML = `⚠️ <b>Chưa kết nối Local Daemon:</b> ${{lastErr}}`;
                }}
